...
function App() {
  ...
  const usesCompactLessonLayout = course.id === 'design-patterns' || lesson.blocks.some((block) => block.type === 'html')
  ...
  const speakNarration = useCallback((text: string, force = false) => {
    ...
    const launch = () => {
      ...
      const koreanVoice = voices.find((voice) => voice.name.toLowerCase().includes('yuna'))
  ...
  useEffect(() => {
    const receiveNarration = (event: MessageEvent) => {
      ...
      const visibleText = typeof event.data?.text === 'string' ? event.data.text.trim() : ''
      narrationTextRef.current = visibleText
      setNarrationStep(step)
      speakNarration(visibleText || lesson.narration?.[step] || lesson.title)
    }
  ...
  useEffect(() => {
    const lessonKey = `${courseId}:${current}`
    if (narrationLessonRef.current === lessonKey) return
    narrationLessonRef.current = lessonKey
    ...
    const hasEmbeddedHtml = lesson.blocks.some((block) => block.type === 'html')
    if (started && narrationEnabled && lesson.narration?.length && !hasEmbeddedHtml) {
      speechTimerRef.current = window.setTimeout(() => speakNarration(lesson.narration?.[0] ?? lesson.title, true), 300)
    }
  }, [courseId, current, lesson, narrationEnabled, speakNarration, started])
  ...
  const toggleNarration = () => {
    ...
    speakNarration(narrationTextRef.current || lesson.narration?.[narrationStep] || `${lesson.title} 슬라이드입니다.`, true)
  }
  ...
  const toggleNarrationPause = () => {
    ...
    if (window.speechSynthesis.paused) {
      ...
      speakNarration(narrationTextRef.current || lesson.narration?.[narrationStep] || lesson.title, true)
    }
  }
